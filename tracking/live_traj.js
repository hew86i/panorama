...
function CheckRow(_t, _max, _num) {
    if (_t == "All") {
    	if(_num == 1 && parseInt($('#trajectoryvalue').val(), 10) > 180)
    	{
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(_num == 1)
            {
				...
				//trajectoryslider
				$('#trajectoryslider').mousemove(function (event) { ShowPopup(event, 'Должина на траекторија во минути<br>Над 180 минути, можен е избор само на едно возило') });
...
function Applay_1(_num, _id) {
    ...
    if(parseInt(traceForUser, 10) > 180)
    {
