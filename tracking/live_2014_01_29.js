...
function AddDetailButton(el, num) {
    var layerCB = Create($(el).children()[0], 'div', 'div-layer-hidedetail-' + num);
    $(layerCB).css({ display: 'block', position: 'absolute', zIndex: '7000', backgroundColor: 'transparent', color: '#fff', left: (document.body.clientWidth - 100) + 'px', top: '35px', width: '100px', height: 'auto', textAlign: 'left' });
    layerCB.className = 'corner15 text3';
    ...
    $(AddCPBtn).click(function (event) {
        if ($('#div-layer-baloon-0').css('display') == 'none') {
            $('#div-layer-baloon-0').css({display: 'block'})
            $('#div-detail-ballon').html(dic('hide', lang));
            $('#div-detail-ballon').css({ borderRadius: '10px 10px 0 0' });
            setCookie("detail_"+_userId, "1", 14);
        } else {
            $('#div-layer-baloon-0').css({display: 'none'})
...
function MoveMarker(_vehicleNo, newLon, newLat, _color, map0, map1, map2, map3, _dt, _status) {
    for (var i = 0; i < Vehicles.length; i++) {
        for (var j = 0; j < 4; j++) {
            Timers[j] = [];
            if (Boards[j] != null) {
                if ((Vehicles[i].ID == _vehicleNo) && (Vehicles[i].Map == j)) {
					...
					$('#img-vehicle-' + _vehicleNo).remove();
					if(_status == '0')
					{
						//Vehicles[i].Marker.events.element.innerHTML += ;
						$($(Vehicles[i].Marker.events.element).children()[0]).append('<div id="img-vehicle-' + _vehicleNo + '" style="height: 16px; background-image: url(\'../images/nosignal.png\'); position: absolute; width: 16px; z-index: 9; left: 12px; top: 12px;"></div>');
					}
...
function MoveMarkerBack(_vehicleNo, newLon, newLat, _color, map0, map1, map2, map3, _dt, _status) {
    for (var i = 0; i < Vehicles.length; i++) {
        for (var j = 0; j < 5; j++) {
            Timers[j] = [];
            if (Boards[j] != null) {
	    ...
	    if (xmlHttp.readyState == 4) {
			...
			for (var j = 0; j < Car.length; j++) {
	        	if(!Car[j].same || $('#vh-small-'+Car[j].id).attr('class').indexOf('Gray') != -1)
        		{
        			...
        			MoveMarker(Car[j].id, parseFloat(Car[j].lon), parseFloat(Car[j].lat), Car[j].color, Car[j].map0, Car[j].map1, Car[j].map2, Car[j].map3, Car[j].datum, Car[j].status);
	            }
