...
function ParseCarStr_rec() {
    ...
    //for (var i=1; i<c.length; i++){
	    var p = c[1].split("|")
	    var _car = new CarType_rec(p[0], p[3], p[1], p[2], p[4], 1);
	    Car[Car.length] = _car
    //}
}
...
function ButtonAddEditPOIokClick() {
    ...
    if (document.getElementById("groupidTEst").title != '')
        ...
        if ($('#btnAddPOI').val() == dic("Update", lang)) {
            ...
            //return false;
            $.ajax({
                url: "EditPoiNew.php?lat=" + $('#poiLat').val() + "&lon=" + $('#poiLon').val() + "&name=" + $('#poiName').val() + "&avail=" + avail + "&ppgid=" + _title + "&id=" + $('#idPoi').val() + "&description=" + $('#poiAddress').val() + "&additional=" + $('#additionalInfo').val() + "&l=" + lang + "&radius=" + _radius,
                context: document.body,
                success: function (data) {
            ...
            //return false;
            $.ajax({
                url: "AddPoi.php?lat=" + $('#poiLat').val() + "&lon=" + $('#poiLon').val() + "&name=" + $('#poiName').val() + "&avail=" + avail + "&ppgid=" + _title + "&description=" + $('#poiAddress').val() + "&additional=" + $('#additionalInfo').val() + "&l=" + lang + "&radius=" + _radius,
                context: document.body,
                success: function (data) {
                    if (data.indexOf("Error") == -1) {
                        msgbox(data.split("@@%%")[2]);
...
function mapEvent(event) {
 ...
 function UpdateGeoFence() {
     ...
     $('#div-enter-zone-name').dialog({ modal: true, zIndex: 9999, width: 642, height: 575, resizable: false,
         buttons: 
         [
            {
        ...
        if (document.getElementById('f-vehicle-' + num + '--1').checked) {
            ...
            for (var i = 0; i < Vehicles.length; i++)
            {
                if (Vehicles[i].Map == num) {
	                ...
	                if(Vehicles[i].Marker.events.element.children[3] != undefined)
                	{
                		$(Vehicles[i].Marker.events.element.children[3]).remove();
        				$(Vehicles[i].Marker.events.element.children[2]).remove();
    				}
                	Vehicles[i].Marker.events.element.innerHTML += '<div class="impuls1_'+num+' gnMarkerBorder' + Vehicles[i].Color + '"></div><div class="impuls3_'+num+' gnMarkerPulsing' + Vehicles[i].Color + '"></div>';
                }
           }
...
function CreateVehicle(_mapNo, _vehicleNo, _color, _lon, _lat, map0, map1, map2, map3, _reg){
	...
	} else
	{
		...
		if(el.children[3] != undefined)
		{
			el.children[2].className = "impuls1_0 gnMarkerBorder" + MarkerColor;
			el.children[3].className = "impuls3_0 gnMarkerPulsing" + MarkerColor;
		}
	}
...
function switchClass12(num)
{
	//return false;
	if(FollowActive[num])
	{
		if($(".impuls1_"+num)[0] != undefined)
		{
		...
		setTimeout("switchClass12("+num+")", 1200);
	} else
	{
		$(".impuls1_"+num).css({ visibility: 'hidden' });
		$(".impuls2_"+num).css({ visibility: 'hidden' });
		$(".impuls3_"+num).css({ visibility: 'hidden' });
		$(".impuls4_"+num).css({ visibility: 'hidden' });
	}
}
        ...
        for (var j = 0; j < 5; j++) {
            ...
            if (Boards[j] != null) {
                if ((Vehicles[i].ID == _vehicleNo) && (Vehicles[i].Map == j)) {
                    ...
                    if(Vehicles[i].el.children[3] != undefined)
					{
						Vehicles[i].el.children[2].className = "impuls1_0 gnMarkerBorder" + _color;
						Vehicles[i].el.children[3].className = "impuls3_0 gnMarkerPulsing" + _color;
					}
	    ...
	    if (xmlHttp.readyState == 4) {
			...
			if(str.indexOf("@%@^") != -1)
			{
				...
				for (var j = 1; j < tourch.length; j++) {
					...
					strAR = "Реден Број: <span style='color:#000'>" + arrRow[4] + "</span>&nbsp;&nbsp;&nbsp;Пристигнување: <span style='color:#040'>" + arrRow[1] + "</span>&nbsp;&nbsp;&nbsp;Поаѓање: <span style='color:#800'>" + arrRow[2] + "</span>&nbsp;&nbsp;&nbsp;Стоење: <span style='color:#800'>" + arrRow[3] + "</span>";
