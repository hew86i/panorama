    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 1; i < _groups.length; i++) {
                ...
                //var _bgimg = 'http://92.55.94.3/new/pin/?color=' + _groups[i].split("|")[2] + '&type=0';
...
function ChangeIconsColor(_color) {
    for (var p = 0; p < 22; p++)
        document.getElementById("GroupIconImg" + p).src = 'http://92.55.94.3/new/pin/?color=' + _color + '&type=' + p;
...
function AddGroup(_tbl) {
    ...
    $("#clickAny").val('');
    for (var p = 0; p < 22; p++)
        document.getElementById("GroupIconImg" + p).src = 'http://92.55.94.3/new/pin/?color=ffffff&type=' + p;
    $('#btnAddGroup').button();
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.indexOf("Error") == -1) {
                    ...
                    var _bgimg = 'http://92.55.94.3/new/pin/?color=' + $("#clickAny").val().substring(1, $("#clickAny").val().length) + '&type=' + _img;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.indexOf(dic("Error", lang)) == -1) {
                        var _bgimg = 'url("http://92.55.94.3/new/pin/?color=' + String(_col.substring(1, _col.length)) + '&type='+_img+'")';
                        for (var i = 0; i < 4; i++)
                            if (Boards[i] != null) {
                            	if($('#numPoi').val() != -1){
                    ...
                    if (data.indexOf("Error") == -1) {
                        ...
                        for (var z = 0; z < Maps.length; z++) {
                            if (Boards[z] != null) {
                                ...
                                var _bgimg = 'http://92.55.94.3/new/pin/?color=' +  _ppp[7] + '&type=' + _ppp[13];
...
function CreateVehicle(_mapNo, _vehicleNo, _color, _lon, _lat, map0, map1, map2, map3, _reg, _status, _speed, _nocomm){
	...
	if(_status == '0')
	{
		...
		$($(el).children()[0]).append('<div id="img-vehicle-' + _vehicleNo + '" style="height: 16px; background-image: url(\'../images/nosignal.png\'); position: absolute; width: 16px; z-index: 9; left: 12px; top: 12px;"></div>');
	} else
	{
		if(_nocomm)
		{
			$($(el).children()[0]).append('<div id="img-vehicle-' + _vehicleNo + '" style="height: 16px; background-image: url(\'../images/nocommunication.png\'); position: absolute; width: 16px; z-index: 9; left: 12px; top: 12px;"></div>');
		}
	}
            ...
            if (Boards[j] != null) {
                if ((Vehicles[i].ID == _vehicleNo) && (Vehicles[i].Map == j)) {
					...
					} else
					{
						if(_nocomm)
						{
							$($(Vehicles[i].Marker.events.element).children()[0]).append('<div id="img-vehicle-' + _vehicleNo + '" style="height: 16px; background-image: url(\'../images/nocommunication.png\'); position: absolute; width: 16px; z-index: 9; left: 12px; top: 12px;"></div>');
						}
					}
                    ...
                    if (Boards[i] != null) {
                        ...
                        for (var j = 1; j < _pp.length; j++) {
                            ...
                            /*if (_ppp[7] == "1")
                                ...
                                var _color = _ppp[7];*/
                            var _bgimg = 'http://92.55.94.3/new/pin/?color=' + _ppp[5] + '&type=' + _ppp[9];
                            //debugger;
                            if(_ppp[3] != '1')
                    ...
                    if (Boards[i] != null) {
                        ...
                        for (var j = 1; j < _pp.length; j++) {
                            ...
                            /*if (_ppp[7] == "1")
                                ...
                                var _color = _ppp[7];*/
                            var _bgimg = 'http://92.55.94.3/new/pin/?color=' + _ppp[5] + '&type=0';
                            //debugger;
                            //tmpMarkers[i][tmpMarkers[i].length - 1].events.element.innerHTML = '<div style="background: transparent url(' + _bgimg + ') no-repeat; width: 24px; height: 24px; font-size:4px"></div>';
                            tmpMarkers[i][tmpMarkers[i].length - 1].events.element.style.cursor = 'pointer';
                            //tmpMarkers[tmpMarkers.length - 1].events.element.style.backgroundColor = '#' + _ppp[7];
