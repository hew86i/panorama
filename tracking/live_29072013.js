                            ...
                            [
                                {
                                    ...
                                    click: function () {
                                        ShowWait();
                                        $.ajax({
                                            url: "checkPassword.php?pass=" + encodeURIComponent($('#alGeoFencePass').val()),
                                            context: document.body,
                                            success: function (data) {
...
function drawRecPerDayNew(_day1, _day2, vh) {
    ...
    $.ajax({
        url: "../report/getHistoryByDayNew.php?v=" + vh + "&sd=" + _d1 + "&ed=" + _d2,
        context: document.body,
        success: function (data) {
...
function MoveMarker(_vehicleNo, newLon, newLat, _color, map0, map1, map2, map3, _dt) {
    for (var i = 0; i < Vehicles.length; i++) {
        for (var j = 0; j < 4; j++) {
            Timers[j] = [];
            if (Boards[j] != null) {
	    ...
	    if (xmlHttp.readyState == 4) {
	        ...
	        for (var j = 0; j < Car.length; j++) {
	        	if(!Car[j].same || $('#vh-small-'+Car[j].id).attr('class').indexOf('Gray') != -1)
        		{
	            	...
	            	MoveMarker(Car[j].id, parseFloat(Car[j].lon), parseFloat(Car[j].lat), Car[j].color, Car[j].map0, Car[j].map1, Car[j].map2, Car[j].map3, Car[j].datum);
	            }
	            ...
	            if (svc != null) {
	                ...
	                if (Car[j].cbfuel == "0.000000" && Car[j].cbrpm == "0.000000" && Car[j].cbtemp == "0.000000" && Car[j].cbdistance == "0.000000")
	                	$('#vh-canbus-' + Car[j].id).css({ display: "none" });
                	else
                	{
                		$('#vh-canbus-' + Car[j].id).css({ display: "block" });
		                if (Car[j].cbfuel == "0.000000")
		                    $('#vh-cbfuel1-' + Car[j].id).css({ display: "none" });
		                else
							{
								$('#vh-cbfuel1-' + Car[j].id).css({ display: "block" });
								$('#vh-cbfuel-' + Car[j].id).html(Math.round(1000*parseInt(Car[j].cbfuel, 10)/100) + ' L');
							}

		                if (Car[j].cbrpm == "0.000000")
		                ...
		                if (Car[j].cbtemp == "0.000000")
		                    $('#vh-cbtemp1-' + Car[j].id).css({ display: "none" });
		                else
							{
								$('#vh-cbtemp1-' + Car[j].id).css({ display: "block" });
								$('#vh-cbtemp-' + Car[j].id).html(Math.round(Car[j].cbtemp));
							}
							
						if (Car[j].cbdistance == "0.000000")
		                ...
		                else
							{
								...
								$('#vh-cbdistance-' + Car[j].id).html(commaSeparateNumber(kilom) + mmetric);
							}
					}
	                ...
	                if(Car[j].zoneids != "")
	                {
	                	for(var z = 0; z < Car[j].zoneids.split(";").length; z++)
	                	{
	                		...
	                		str += '<div style="float:left; width:16px; height:16px; margin-left:2px; margin-bottom:2px; cursor:pointer" onclick="FindVehicleOnMap0(' + Car[j].id + ')" class="' + cname + '">' + Car[j].id + '</div>';
					...
					$('#vh-temp-' + Car[j].id).html(Car[j].temperature + ' °C');
					$('#vh-litres-' + Car[j].id).html(Car[j].litres + ' L');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if($('#IDMarker_' + PointsOfRoute[_len + 1].id + '_' + _len)[0] != undefined)
            {
            	...
            	$('#IDMarker_' + PointsOfRoute[_len + 1].id + '_' + _len)[0].children[1].value = Math.round(kilom * 100)/ 100 + mmetric; //Math.round((parseInt(data.substring(data.indexOf("distance") + 10, data.indexOf("total_time") - 2), 10) / 1000)) + " km";
            	...
            	$('#IDMarker_Total')[0].children[1].value = Math.round(totalkm * 100)/ 100 + mmetric;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if($('#IDMarker_' + PointsOfRoute[parseInt(_ordNum, 10) + 1].id + '_' + _ordNum)[0] != undefined)
            {
	            ...
	            $('#IDMarker_' + PointsOfRoute[parseInt(_ordNum, 10) + 1].id + '_' + _ordNum)[0].children[1].value = Math.round(kilom * 100)/ 100 + mmetric; //Math.round((parseInt(data.substring(data.indexOf("distance") + 10, data.indexOf("total_time") - 2), 10) / 1000)) + " km";
	            //var _min = Math.round((parseInt(data.substring(data.indexOf("total_time") + 12, data.indexOf("start_point") - 2), 10) / 60));
	            var _min = " )";
            	if($('#txt_alertWait').val() != "/")
            		_min = " min )";
