...
<?php
	...
	$dsVehicles = query("select * from vehicles where id in (" . $sqlV. ") order by organisationid asc, cast(code as integer) asc"); 
?>
<body>
<table class="fixed" style="top:17px; position: relative;">
<tr>
...
<div id="vehicle-<?= $drVehicle["id"]?>" style="text-align:left; position:relative; left:5px; top:3px">
	            ...
	            <td <?php if($clientType == 2) $width=15; else $width=15; ?> width=<?=$width?>% style="font-size:11px" align="center" class="corner5 text5 <?=$classTr?>">
		            	...
		            	?>
		            	<span id="spanOd1-<?= $drVehicle["id"]?>" onmousemove="ShowPopup(event, '<span class=\'text5\'>Почетна вредност на одометарот</span>')" onmouseout="HidePopup()"><?=$od1?></span>-<span id="spanOd2-<?= $drVehicle["id"]?>" onmousemove="ShowPopup(event, '<span class=\'text5\'>Моментална вредност на одометарот</span>')" onmouseout="HidePopup()"><?=$od2?></span> <br>Вк.<span id="spanOdTot-<?= $drVehicle["id"]?>" onmousemove="ShowPopup(event, '<span class=\'text5\'>Поминато растојание во текот на денот</span>')" onmouseout="HidePopup()"><?=$odTot?></span>
	            </td>
