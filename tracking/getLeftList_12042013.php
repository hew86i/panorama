	...
	<div id="menu-container-3" style="width:230px; padding-left:10px; padding-bottom:10px; padding-top:10px; overflow:auto">
		<div id="alarms">
			<?php
				...
				$sqlAlarm .= " and ah.datetime > cast((select now()) as date) + cast('-20 day' as interval) ";
				$sqlAlarm .= " order by read asc, datetime desc";
				$dsAlarms = query($sqlAlarm);
				$brojac = 1;
				$brojac1 = dlookup("select count(*) from alarmshistory where vehicleid in (select id from vehicles where clientid=" . session("client_id") . ") and datetime > cast((select now()) as date) + cast('-20 day' as interval) and read='0'");
