...
function AddVehicleToFollow(el, num){
	...
	var str = '<label class="menu_vehicle">&nbsp;<input onChange="VehicleFListClick(' + num + ', -1)" id="f-vehicle-' + num + '--1" type="checkbox" />&nbsp;' + dic("allVehicles", lang) + '</label>'
...
function AddVehicleChooser(el, num) {
    ...
    $(layerList).css({ display: 'block', position: 'absolute', zIndex: '7999', backgroundColor: '#e2ecfa', border: '1px solid #1a6ea5', color: '#fff', padding: '2px 5px 2px 1px', width: '155px', height: '300px', cursor: 'pointer', textAlign: 'center', overflowX: 'hidden', overflowY: 'scroll', display: 'none' })
	...
	for (var i = 0; i < VehicleListOENum.length; i++) {
		str += '<div class="menu_vehicle_OE"><input class="inputOEAll-'+num+'" style="position: absolute;" onChange="VehicleListClickOE(' + num + ', ' + VehicleListOENum[i] + ')" id="cb-vehicleOE-' + num + '-' + VehicleListOENum[i] + '" type="checkbox" /><div style="padding-left: 22px;" class="collapsible'+num+'" id="section-' + num + '-' + (i+1) + '"><strong>' + VehicleListOEName[0].split(",")[i] + '</strong></div><div class="container"><div class="content" id="operation-' + num + '-' + VehicleListOENum[i] + '"></div></div></div>';
