...
function AlertEventHide(_num, _dt, _reg, _type){
...
function AlertEvent(_dt, _reg, _type){
	_num = "1";
	if(_num == "1"){
		$('#div-mainalerts').fadeIn('slow');
		window.clearTimeout(TimerAlarms);
		...
		if ($('#alertsNew').css('visibility') == "hidden")
		{
			...
			var alertIn = '<div onmouseover="opacityIn('+parseInt($('#alertsNew').val(), 10)+')" onmouseout="opacityOut('+parseInt($('#alertsNew').val(), 10)+')" id="div-alerts_'+parseInt($('#alertsNew').val(), 10)+'" class="corner15 shadowalerts" onclick="AlertEventHide('+parseInt($('#alertsNew').val(), 10) + ',\'' +_dt + '\',\'' + _reg + '\',\'' + _type + '\')" style="display: none; cursor: pointer; color: #fff; float: left; '+mt+'; height: 80px; padding-top: 10px; padding-left: 10px; background-color: #387CB0; position: relative; opacity: 0.7; width: 290px;">';
			...
			alertIn += '<strong style="margin-left: 15px;position: relative; top: 5px;">Датум: </strong> <font style="position: relative; top: 5px;"> ' + _dt + '</font></div></div>';
...
function OpenMapAlarm(_dt, _reg, _type){
    $('#dialog-map').html('<iframe id="iframemaps" style="width: 100%; height: 100%; z-index: 9999; border:0px" src=""></iframe>');
    //alert('LoadMap.php?datetime=' + _dt + '&reg=' + _reg + '&type=' + _type + '&l=' + lang);
    //LoadMap.php?datetime=2013-02-26 15:27:18.695&reg=PP-967-CC&type=alarmpanic&l=mk
    //localhost/panorama/tracking/LoadMap.php?datetime=2013-02-26 15:27:18.695&reg=PP-967-CC&type=alarmpanic&l=mk
    document.getElementById('iframemaps').src = 'LoadMap.php?datetime=' + _dt + '&reg=' + _reg + '&type=' + _type + '&l=' + lang;
    $('#dialog-map').dialog({ modal: true, height: 700, width: 1000, zIndex: 9999, beforeClose: function(event, ui) {
