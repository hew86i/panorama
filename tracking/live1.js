...
setTimeout(function(){ 
		if ($('#iFrmS')[0].contentWindow != undefined) {
			/*if (p[42] != "/" && p[42] != "" && p[42] != undefined && p[28] == '1') {
				debugger;
			...
			if (_car.odometar != "") {
				...
				if ($('#iFrmS')[0].contentWindow.$('#spanOd1-'+p[27]).html() != "/" && $('#iFrmS')[0].contentWindow.$('#spanOd1-'+p[27]).html() != undefined) {
...
/*var odometerTotal = parseInt(($('#iFrmS')[0].contentWindow.$('#spanOd2-'+p[27]).html()).toString().replace(',', '').split( )[0]) - parseInt(($('#iFrmS')[0].contentWindow.$('#spanOd1-'+p[27]).html()).toString().replace(',', '').split( )[0]);*/
					...
					if (odometerTotal < 0) odometerTotal = 0;
        ...
        if (svc != null) {
			...
			if(FirstLoad == false){
			    if(kola.driver != undefined)
				 ...
				 //alert(kola.driverdest + " " + kola.id)   
				 if(kola.driverdest != undefined) {
				 	if(kola.driverdest != "/") {
				 		$('#div-driverdest-' + kola.id).show()
				 		$('#vh-driverdest-' + kola.id).html(kola.driverdest)    
				 	} else {
				 		$('#div-driverdest-' + kola.id).hide()
				 	}
				 } else {
				 	$('#div-driverdest-' + kola.id).hide()
				 }				   
			...
			if(allowweather == '1') {
    			if(_podatoci.split("#")[j].split("|")[41] != undefined) {
        			if(_podatoci.split("#")[j].split("|")[41] != "" && _podatoci.split("#")[j].split("|")[41] != "/") {
...
function ConnectToServer(){
	...
	events: {
			...
			fromgarmin: function(e) {
                var feed = e.data+'';
                feed = feed.split("|*&");
                /*$.ajax({
                    url: "../main/SendMess.php?action=garmin&fromid=" + feed[0] + "&toobj=user&subject=&body=" + kescape(feed[1]) + "&dt=" + feed[2] + "&messid=" + feed[3],
                    context: document.body,
                    success: function (data) { 
			...
			camera: function(e) {
			    ...
			    feed = feed.split("|");
                if(feed[1] == 'Connected'){
                	$('#vh-img-newimage').css({display: 'none'});  
                	$('#cameraName').html(feed[3]);
				    $('#cameraTime').html(formatdatetime(feed[4]).split(".")[0]);
				    $('#cameraImg').attr("src", "../cam/"+feed[3]);    
				    
				    $("#cameraNew").attr("disabled", "");
     				$("#cameraNew").css({cursor: 'pointer', color: ''});
     	
     				var cl = $('#camera-'+feed[0]).get(0).attributes.onclick.nodeValue;
					var cl1 = cl.split(',')[0] + ',' + cl.split(',')[1] + ', ' + '\''+feed[3]+'\'' + ', ' + '\''+formatdatetime(feed[4]).split(".")[0]+'\'' + ', ' + (parseInt(cl.split(',')[4].split(")")[0])+1) + ")";
					$('#camera-'+feed[0]).get(0).attributes.onclick.nodeValue = cl1;
					$('#camNo').html(parseInt(cl.split(',')[4].split(")")[0])+1);
					
					if (parseInt(cl.split(',')[4].split(")")[0])+1 >= 3) {
			...
			garmin: function(e) {
			    var feed = e.data+'';
			    feed = feed.split("|");
			    if(feed[1] == 'MessageSent'){
			        if($('#succMess')[0] != undefined) {
                ...
                if(feed[1] == 'etastatus'){
                    if($('#vh-img-garminref-' + feed[2])[0] != undefined) {
                        ...
                        if(parseInt(parseInt(feed[4], 10) / 1000, 10) > 0)
                            $('#vh-garmin-estimationkm-' + feed[2]).html(parseFloat(parseInt(feed[4], 10) / 1000).toFixed(1) + ' Km');
                        else
                            $('#vh-garmin-estimationkm-' + feed[2]).html(parseInt(feed[4], 10) + ' m');
                    }
                }
                if(feed[1] == 'StopPositionEnd'){
                    var _id = 'garminstop-' + feed[2];
                    $('#' + _id).css({display: 'none'});
                    if($('#garminList-' + feed[0] + '-' + feed[3])[0] != undefined) {
                        $('#garminList-' + feed[0] + '-' + feed[3]).css({opacity: '1'})
                ...
                if(feed[1] == 'StopPositionStart'){
                    var _id = 'garminstop-' + feed[2];
                    $('#' + _id).load('./getGarminStopPos.php?gid=' + feed[3], function() {
                ...
                if(feed[1] == 'StopPositionUnRead' || feed[1] == 'StopPositionRead' || feed[1] == 'StopPositionDelete'){
                    if($('#garminList-' + feed[0] + '-' + feed[3])[0] != undefined) {
                        if(feed[1] == 'StopPositionDelete')
                            $('#garminList-' + feed[0] + '-' + feed[3]).css({opacity: '0.5'})
                        else
                            $('#garminList-' + feed[0] + '-' + feed[3]).css({opacity: '1'})
                        $('#garminList-' + feed[0] + '-' + feed[3]).load('./getGarminInfo.php?gid=' + feed[3]);
                    }
                ...
                if(feed[1] == 'NotConnected'){
                    ...
                    if($('#predefmess')[0] != undefined) {
                        $('#predefmess').load('../main/PredefinedMess.php?id=' + feed[0], function(){
                            $("#txtCannedReg").load('./garminReloadReg.php?id=' + feed[0], function(){
                                HideWait(); 
                            });
                        });
                    }
                ...
                if(feed[1] == 'InsertQuickMessage' || feed[1] == 'DeleteQuickMessage'){
                    if($('#predefmess')[0] != undefined) {
                        $('#predefmess').load('../main/PredefinedMess.php?id=' + feed[0], function(){
                            $("#txtCannedReg").load('../tracking/garminReloadReg.php?id=' + feed[0], function(){
                                HideWait(); 
                            });
                        });
                    } else {
...
function IsConnected(){
	if (ws==null){
		...
		if(ddiff>5*1000){
			ConnectionLost(dic("LostConnection"));
			ws.close();
			ws = null
		}
	}
	    ...
	    success: function (data) {
			...
			for (var i = 0; i < Car.length; i++) {
				...
				if($('#vh-date-' + kola.id).css("color") != "rgb(255, 0, 0)")
				{
                    ...
                    if(clientid == "459") {
                        if(parseInt(_date.split(' ')[1].split(':')[0], 10) > 12) {
                            var _h = (_date.split(' ')[1].split(':')[0] - 12);
                            _date = _date.split(' ')[0] + ' ' + (_h < 10 ? '0' + _h : _h) + ':' + _date.split(' ')[1].split(':')[1] + ':' + _date.split(' ')[1].split(':')[2];
                        }
                        _date1 = formatdatetimeusa(_date1);
                    }
...
function shleft() {
    if (parseInt($('#race-img').css('left'), 10) > 200) {
        document.getElementById("vehicle-list").style.width = '1px';
        var w = document.body.clientWidth
        $('#searchVeh').css({ width: '1px' });
        $('#searchbyreg').css({ display: 'none' });
        $('#div-menu').css({ display: 'none' });
        $('#div-map').css({ width: (w - 3) + 'px' });
        $('#top1').css({left: '12px', width: (w - 26) + 'px'})
        var _l1 = 3;
        var _w = new Array();
        ...
        var w = document.body.clientWidth;
        $('#top1').css({left: '290px', width: (w - 306) + 'px'})
        var _w = new Array();
        if (SelectedSpliter == "0")
            _w[0] = -6;
        else
...
function fncAddCanned(_id, _gsmnum){
    $('#addCanned').html('');
    $('#addCanned').load('../main/CannedForm.php?id=' + _id + '&gsmnum=' + _gsmnum);
    $('#addCanned').dialog({ modal: true, width: 700, height: 600, resizable: false, zIndex: 9998,
        buttons: 
        [{
            text: dic("Tracking.Close"),
            click: function() {
                $(this).dialog("close");
            }
        }]
...
function promeniOdometar(id, code){
    ...
    if(metric == 'mi') {
        odometarVrednost = Math.round(odometarVrednost * 1.60934);
    }
	if(odometarVrednost=="")
	{
		        	...
		        	[{
					    ...
					    click: function() {
						        ...
						        success: function(data){
						        	...
						        	var currkm = commaSeparateNumber($('#txtOdometer').val().replace(/\,/g,'').replace(/\./g,'')) + mmetric;
