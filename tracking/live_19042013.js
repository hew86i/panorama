...
var ArrLineFeature;
function DrawPath_Rec(LonArray, LatArray, DistArray, DTArray, alphaArray, alarmsArray, idlingArray, DTArrayDiff, vhID, VehReg) {
...
function drawRecPerDay(_day1, _day2, vh) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            DrawPath_Rec(strLon1, strLat1, strDist, strdiffDT, strAlpha, strAlarms, strIdling, strdiffDT1, Car[0].id, Car[0].reg);
            pointType = []
...
function CreateMarkerStartEnd(_lonn, _latt, _time, _date, _who) {
    ...
    if (Maps[0] != null) {
        ...
        var popup = new OpenLayers.Popup.FramedCloud("Popup",
            ll,
            new OpenLayers.Size(500, 500),
            "<div class='text1' style='overflow: hidden; font-size:.8em; position: relative; left: -2px; top: -3px; width: 125px; height: 35px; margin-right: -15px; margin-bottom: -22px;'><strong style='font-size: 12px;'><img onmousemove='ShowPopup(event, \"" + _text + "\")' onmouseout='HidePopup()' src='../images/" + imgSE + ".png' style='height: 17px; width: 16px; position: relative; top: 4px;' />: " + _date.split(/\-|\s/)[3].split(":")[0] + ":" + _date.split(/\-|\s/)[3].split(":")[1] + " " + _date.split(/\-|\s/)[2] + "-" + _date.split(/\-|\s/)[1]+ "-" + _date.split(/\-|\s/)[0] + "</strong></div>", null, true);
        this.popup = popup;
        this.popup.contentDiv.style.overflow = 'hidden';
        map.addPopup(this.popup);
        this.popup.div.style.opacity = '0.7';
