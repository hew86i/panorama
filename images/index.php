...
<?php
	...
	$dsTraceSnooze = query("select trace, snooze, metric, weather from users where id=" . session("user_id"));
...
<div id="div-Add-Garmin" style="display: none;">
    ...
    <br /><br />
	<div align="right" style="display:block; width:380px; height: 30px; padding-top: 5px; position: relative; float: right; right: 15px;">
		<span id="errAddGarmin" style="display: none; position: absolute; color: red; text-align: left; margin-left: 51px;">Ги немате внесено<br>потребните информаци!</span>
...
<div id="div-Directions" class="text2_ corner5 shadow" style="display: none; opacity: 0.9; z-index: 9996; background-color: white; float: right; position: absolute; right: 20px; top: 55px; width: 310px; min-height: 315px; height: auto; padding-bottom: 10px;">
	...
	<table class="text2_" style="font-size: 11px; color: rgb(65, 65, 65); margin-top: 5px; font-family: Arial,Helvetica,sans-serif; margin-left: 20px;" cellpadding="0" cellspacing="0" border="0">
		...
		<tr height="67px">
    		<td>
    			<input id="fromDirection" onmousemove="ShowPopup(event, '<?=dic('search')?>' + ' ' + '<?=dic('Reports.Mo')?>'.toLowerCase() + ' ' + '<?=dic('Tracking.Address')?>'.toLowerCase())" onmouseout="HidePopup()" type="text" class="textboxCalender corner5 text2_" placeholder="<?=dic('ChooseStartingPoint')?>" style="border-bottom: 0px none; width: 237px; border-radius: 5px 0px 0px; border-right: 0px none;" />
		...
		<tr style="height:5px;">
	         <td>
	         	...
	         	<img onclick="SwitchPoints()" width="8" height="11" src="../images/upMenu1.png" style="cursor: pointer; float: right; opacity: 0.25; position: absolute; margin-top: -5px; right: 7px;">
	         </td>
	    </tr>
	    <tr height="67px">
    		<td>
    			<input id="toDirection" onmousemove="ShowPopup(event, '<?=dic('search')?>' + ' ' + '<?=dic('Reports.Mo')?>'.toLowerCase() + ' ' + '<?=dic('Tracking.Address')?>'.toLowerCase())" onmouseout="HidePopup()" type="text" class="textboxCalender corner5 text2_" placeholder="<?=dic('ChooseDestination')?>" style="border-bottom: 0px none; width: 237px; border-radius: 5px 0px 0px; border-right: 0px none;" />
		...
		<tr style="height:5px;">
	         <td>
	         	<table id="ResultDirectionsTable" cellpadding="0" cellspacing="0" class="text2_" style="display: none; padding: 7px;">
	         		<tr>
	         			<td style="border-bottom: 1px dotted #2f5185"><?=dic('ShortRoute')?></td>
	         			<td style="border-bottom: 1px dotted #2f5185; border-left: 1px dotted #2f5185; padding-left: 5px;"><?=dic('FastRoute')?></td>
	         		</tr>
	         		<tr>
    ...
    <br />
	<div align="right" style="display:block; width:380px; height: 30px; padding-top: 5px; position: relative; float: right; right: 20px;">
		<span id="errDirections" style="display: none; position: absolute; color: red; text-align: left; margin-left: 51px;"><?=dic('Admin.completeAllfiels')?></span>
        <img id="loadingDirections" style="display: none; width: 140px; position: absolute; left: 10px; margin-top: 7px;" src="../images/loading_bar1.gif" alt="" />
		<input type="button" style="position: relative; float: right;" class="BlackText corner5" id="btnCancelDirections" value="<?php echo dic("Tracking.Close")?>" onclick="$('#div-Directions').css({display: 'none'}); clearDirectionS(); clearDirectionE();" />        
...
<script type="text/javascript">
    ...
    $("#toDirection").autocomplete({
        ...
        open: function () { $("#toDirection").autocomplete("widget").width(269) }
    });
    $("#fromDirection").autocomplete({
        ...
        open: function () { $("#fromDirection").autocomplete("widget").width(269) }
    });
