...
function ButtonAddGroupOkClick() {
    if (($('#GroupName').val() != '') && ($('#clickAny').val() != '')) {
        ...
        for (var i = 0; i < 1; i++)
            ...
            $.ajax({
                url: "AddGroupNew.php?groupName=" + String($('#GroupName').val()) + "&fcolor=" + String($("#clickAny").val().substring(1, $("#clickAny").val().length)) + "&img=" + _img + "&l=" + lang,
                context: document.body,
                success: function (data) {
                    if (data.indexOf("Error") == -1) {
                        ...
                        var _bgimg = 'http://gps.mk/new/pin/?color=' + $("#clickAny").val().substring(1, $("#clickAny").val().length) + '&type=0';
...
function ButtonAddEditPOIokClick() {
    ...
    if (document.getElementById("groupidTEst").title != '')
        ...
        if ($('#btnAddPOI').val() == dic("Update", lang)) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.indexOf(dic("Error", lang)) == -1) {
                        //var _bgimg = 'url("http://gps.mk/new/pin/?color=' + String(_col.substring(1, _col.length)) + '&type=0")';
                        for (var i = 0; i < 4; i++)
                            if (Boards[i] != null) {
                            	if($('#numPoi').val() != -1){
            ...
            //return false;
            $.ajax({
                url: "AddPoi.php?lat=" + $('#poiLat').val() + "&lon=" + $('#poiLon').val() + "&name=" + $('#poiName').val() + "&avail=" + avail + "&ppgid=" + _title + "&description=" + $('#poiAddress').val() + "&additional=&l=" + lang + "&radius=" + _radius,
                context: document.body,
                success: function (data) {
                    if (data.indexOf("Error") == -1) {
                        msgbox(data.split("@@%%")[2]);
                        ...
                        for (var z = 0; z < Maps.length; z++) {
                            if (Boards[z] != null) {
                                ...
                                tmpMarkers[z][tmpMarkers[z].length - 1].events.element.setAttribute("onmousemove", "ShowPopup(event, '<strong style=\"font-size: 14px;\">" + _ppp[2] + "<br /></strong>" + dic("Group", lang) + ": <strong style=\"font-size: 12px;\">" + _ppp[8] + "</strong>')")
...
function mapEvent(event) {
                                        ...
                                        $.ajax({
                                            ...
                                            success: function (data) {
                                                if (data.indexOf("Wrong") != -1) {
                                                    ...
                                                    UpdateArea($('#txt_zonename').val(), strPoints, selectedFeature[SelectedBoard].style.areaid, avail, $("#gfGroup dt a")[0].title, _col);
 ...
 function onFeatureSelect(feature) {
     if (feature.geometry.id.indexOf("LineString") == -1) {
         ...
         $(PolygonMenu).css({ display: 'block', position: 'absolute', zIndex: '7999', backgroundColor: '#387cb0', color: '#fff', left: '50px', top: '50px', padding: '12px 15px 12px 15px', width: lang == 'en' ? '137' : '177' + 'px', cursor: 'pointer', textAlign: 'left' });
 ...
 function LoadMaps() {
     for (var i = 0; i < 4; i++) {
         if (Boards[i] != null) {
			 ...
			 //GLOBSY MAPS
			 if (MapType[i] == 'YAHOOM') { layer = new OpenLayers.Layer.WMS( "OpenLayers WMS", "http://maps.gps.mk:8080/geoserver/gwc/service/wms", {'layers': 'GeonetMaps', format: 'image/png'}); }
...
function DrawPathAgain(LonArray, LatArray, vhID, _j, _i, VehReg) {
    ...
    if (PathPerVeh[vhID] == undefined || PathPerVeh[vhID] == "") {
        ...
        for (i in _lon) {
            ...
            styles.push({ 'strokeWidth': debelina, 'strokeColor': '#0000FF', 'strokeOpacity': opac, 'VehID': dic("Number", lang) + ' ' + vhID + '<br />' + VehReg + '<br />' + CarDT })
            points.push(point)
...
//        	debugger;
    	...
    	if(PathPerVeh[vhID][0].style.VehID.split("<br />").length == 2)
    	{
        	vectors[_j].removeFeatures(PathPerVeh[vhID][0]);
        	PathPerVeh[vhID] = PathPerVeh[vhID].splice(1, PathPerVeh[vhID].length);
        }
        //if ((PathPerVeh[vhID].length / 25) > 10) {
        var boolCheck = true;
        while(boolCheck)
        {
        	var _dt = PathPerVeh[vhID][0].style.VehID.substring(PathPerVeh[vhID][0].style.VehID.lastIndexOf("<br />")+6, PathPerVeh[vhID][0].style.VehID.length);
...
function DrawPath1(LonArray, LatArray, DistArray, dtArray, alphaArray, vhID, VehReg) {
	...
	var points = new Array();
	var styles =  new Array(); //{'strokeWidth': '4', 'strokeColor': '#0000FF', 'strokeOpacity': '0.8', 'VehID': dic("Number", lang) + ' ' + vhID + '<br />' + VehReg + '<br />' +  };
	var cir = _lon.length / 3
	var cir1 = cir + cir
	var cir2 = _lon.length
	var numA = 0;
	for (var z = 0; z < Maps.length; z++) {
        ...
        styles.push({ 'strokeWidth': '4', 'strokeColor': '#0000FF', 'strokeOpacity': '1', 'VehID': dic("Number", lang) + ' ' + vhID + '<br />' + VehReg + '<br />' + dtArray.split(",")[1] });
        points.push(point);
	    ...
	    for (var i = 2; i < _lon.length - 1; i++) {
	    	if((parseFloat(DistArray.split(",")[i]) < 2500))
			{
		        ...
		        styles.push({ 'strokeWidth': '4', 'strokeColor': '#0000FF', 'strokeOpacity': '1', 'VehID': dic("Number", lang) + ' ' + vhID + '<br />' + VehReg + '<br />' + dtArray.split(",")[i] });
		        points.push(point);
		        ...
		        if(numA == 10)
		        {
				    ...
				    if(parseInt(alphaArray.split(",")[i], 10) >= 22 && parseInt(alphaArray.split(",")[i], 10) < 67)
				    	iconAlpha = "sDesnoGore";
			    	else
			    		if(parseInt(alphaArray.split(",")[i], 10) >= 67 && parseInt(alphaArray.split(",")[i], 10) < 112)
				    		iconAlpha = "sGore";
			    		else
				    		if(parseInt(alphaArray.split(",")[i], 10) >= 112 && parseInt(alphaArray.split(",")[i], 10) < 157)
				    			iconAlpha = "sLevoGore";
							else
					    		if(parseInt(alphaArray.split(",")[i], 10) >= 157 && parseInt(alphaArray.split(",")[i], 10) < 202)
					    			iconAlpha = "sLevo";
				    			else
					    			if(parseInt(alphaArray.split(",")[i], 10) >= 202 && parseInt(alphaArray.split(",")[i], 10) < 247)
					    				iconAlpha = "sLevoDolu";
				    				else
						    			if(parseInt(alphaArray.split(",")[i], 10) >= 247 && parseInt(alphaArray.split(",")[i], 10) < 292)
						    				iconAlpha = "sDole";
					    				else
						    				if(parseInt(alphaArray.split(",")[i], 10) >= 292 && parseInt(alphaArray.split(",")[i], 10) < 337)
						    					iconAlpha = "sDesnoDolu";
					    					else
						    					if(parseInt(alphaArray.split(",")[i], 10) >= 337 && parseInt(alphaArray.split(",")[i], 10) < 22)
						    						iconAlpha = "sDesno";
			        var icon = new OpenLayers.Icon('../images/' + iconAlpha + '.png', size, null, calculateOffset);
				    if (Maps[z] != null) {
				        var ll = new OpenLayers.LonLat(parseFloat(_lon[i]), parseFloat(_lat[i])).transform(new OpenLayers.Projection("EPSG:4326"), Maps[z].getProjectionObject())
				        ...
				        tmpMarkersRec[tmpMarkersRec.length - 1].events.element.setAttribute("onmousemove", "ShowPopup(event, '<strong >" + dic("Number", lang) + ' ' + vhID + '<br />' + VehReg + '<br />' + dtArray.split(",")[i] + "</strong>')");
	    ...
	    for (var j = 0; j < Car.length; j++) {
	        if (Car[j].id == vhID) {
	            ...
	            styles.push({ 'strokeWidth': 4, 'strokeColor': '#0000FF', 'strokeOpacity': '1', 'VehID': dic("Number", lang) + ' ' + vhID + '<br />' + VehReg + '<br />' + Car[j].datum.split(" ")[1] + " " + Car[j].datum.split(" ")[0] });
	            points.push(point);
	            var lineString1 = new OpenLayers.Geometry.LineString([points[i - 2], points[i - 1]]);
		        var lineFeature1 = new OpenLayers.Feature.Vector(lineString1, null, styles[i - 2]);
		        vectors[z].addFeatures([lineFeature1]);
...
///			debugger; ((parseInt(diffDTArray.split(",")[_j], 10) * 40) < 6000) || 
		if((parseFloat(DistArray.split(",")[_j]) < 2500))
		{
	        ...
	        if(numA == 20)
	        {
			    ...
			    var calculateOffset = function (size) { return new OpenLayers.Pixel(-(size.w / 2), -(size.h / 2)); };
			    var iconAlpha = "sDesno";
			    if(parseInt(alphaArray.split(",")[_j], 10) >= 22 && parseInt(alphaArray.split(",")[_j], 10) < 67)
			    	iconAlpha = "sDesnoGore";
		    	else
		    		if(parseInt(alphaArray.split(",")[_j], 10) >= 67 && parseInt(alphaArray.split(",")[_j], 10) < 112)
			    		iconAlpha = "sGore";
		    		else
			    		if(parseInt(alphaArray.split(",")[_j], 10) >= 112 && parseInt(alphaArray.split(",")[_j], 10) < 157)
			    			iconAlpha = "sLevoGore";
						else
				    		if(parseInt(alphaArray.split(",")[_j], 10) >= 157 && parseInt(alphaArray.split(",")[_j], 10) < 202)
				    			iconAlpha = "sLevo";
			    			else
				    			if(parseInt(alphaArray.split(",")[_j], 10) >= 202 && parseInt(alphaArray.split(",")[_j], 10) < 247)
				    				iconAlpha = "sLevoDolu";
			    				else
					    			if(parseInt(alphaArray.split(",")[_j], 10) >= 247 && parseInt(alphaArray.split(",")[_j], 10) < 292)
					    				iconAlpha = "sDole";
				    				else
					    				if(parseInt(alphaArray.split(",")[_j], 10) >= 292 && parseInt(alphaArray.split(",")[_j], 10) < 337)
					    					iconAlpha = "sDesnoDolu";
				    					else
					    					if(parseInt(alphaArray.split(",")[_j], 10) >= 337 && parseInt(alphaArray.split(",")[_j], 10) < 22)
					    						iconAlpha = "sDesno";
		        var icon = new OpenLayers.Icon('../images/' + iconAlpha + '.png', size, null, calculateOffset);
			    if (Maps[0] != null) {
	        ...
	        if(alarmsArray.split(",")[_j] != "")
	        {
			    ...
			    var calculateOffset = function (size) { return new OpenLayers.Pixel(-(size.w / 2), -(size.h / 2)); };
		        var icon = new OpenLayers.Icon('../images/alarm1.png', size, null, calculateOffset);
			    if (Maps[0] != null) {
			        var ll = new OpenLayers.LonLat(parseFloat(_lon[_j]), parseFloat(_lat[_j])).transform(new OpenLayers.Projection("EPSG:4326"), Maps[0].getProjectionObject())
			        //if (MapType[0] == 'YAHOOM') { var ll = new OpenLayers.LonLat(parseFloat(_lonn), parseFloat(_latt)) }
			        var MyMar = new OpenLayers.Marker(ll, icon);
	        ...
	        {
	        	CreateMarkerIgnition_RecNew(_lon[_j], _lat[_j], parkingArray.split(",")[_j].split("|")[0].substring(0,parkingArray.split(",")[_j].split("|")[0].indexOf(".")), parkingArray.split(",")[_j].split("|")[1].substring(0,parkingArray.split(",")[_j].split("|")[1].indexOf(".")), DTArrayDiff.split(",")[_j].substring(0,DTArrayDiff.split(",")[_j].indexOf(".")));
	        	/*
		        ...
		        var icon = new OpenLayers.Icon('../images/ParkingIdling1.png', size, null, calculateOffset);
			    if (Maps[0] != null) {
			        var ll = new OpenLayers.LonLat(parseFloat(_lon[_j]), parseFloat(_lat[_j])).transform(new OpenLayers.Projection("EPSG:4326"), Maps[0].getProjectionObject())
			        //if (MapType[0] == 'YAHOOM') { var ll = new OpenLayers.LonLat(parseFloat(_lonn), parseFloat(_latt)) }
			        var MyMar = new OpenLayers.Marker(ll, icon);
	        ...
	        } else
        	{
	        	if(idlingArray.split(",")[_j] != "")
		        {
				    ...
				    var calculateOffset = function (size) { return new OpenLayers.Pixel(-(size.w / 2) - 1, -size.h + 3 ); };
			        var icon = new OpenLayers.Icon('../images/idle1.png', size, null, calculateOffset);
				    if (Maps[0] != null) {
				        var ll = new OpenLayers.LonLat(parseFloat(_lon[_j]), parseFloat(_lat[_j])).transform(new OpenLayers.Projection("EPSG:4326"), Maps[0].getProjectionObject())
				        //if (MapType[0] == 'YAHOOM') { var ll = new OpenLayers.LonLat(parseFloat(_lonn), parseFloat(_latt)) }
				        var MyMar = new OpenLayers.Marker(ll, icon);
		        ...
		        {
		        	//debugger;
		        	CreateMarkerIgnition_Rec(_lon[_j], _lat[_j], parkingArray.split(",")[_j].split("|")[0].substring(0,parkingArray.split(",")[_j].split("|")[0].indexOf(".")), parkingArray.split(",")[_j].split("|")[1].substring(0,parkingArray.split(",")[_j].split("|")[1].indexOf(".")));
		        }
	        }
		} else
		{
...
function CreateMarkerIgnition_RecNew(_lonn, _latt, _date, _date1, _date2) {
    ...
    if (Maps[0] != null) {
        ...
        var markerClick = function (evt) {
            if (this.popup == null) {
                ...
                //"<div class='text1' style='overflow: hidden; font-size:.8em; position: absolute; top: -1px;'><strong style='font-size: 12px;'>Почеток: " + _date1 + "<br />Крај: " + _date + "<br />Вкупно стоење: " + (_diffDT.days != 0 ? (_diffDT.days + " days") : "") + " " + (_diffDT.hours != 0 ? (_diffDT.hours + " h") : "") + " " + (_diffDT.minutes != 0 ? (_diffDT.minutes + " m") : "") + " " + (_diffDT.seconds != 0 ? (_diffDT.seconds + " s") : "") + "</strong></div>", null, true);
                    "<div class='text1' style='overflow: hidden; font-size:.8em; position: relative; left: -2px; top: -8px; width: 125px; height: 135px; margin-right: -15px; margin-bottom: -22px;'><strong style='font-size: 12px;'><img onmousemove='ShowPopup(event, \"" + dic("STOS", lang) + "\")' onmouseout='HidePopup()' src='../images/startRec.png' style='height: 17px; width: 16px; position: relative; top: 4px;' />: " + _NewCreateDateTime1 + "<br /><img onmousemove='ShowPopup(event, \"" + dic("ETOS", lang) + "\")' onmouseout='HidePopup()' src='../images/endRec.png' style='height: 17px; width: 16px; position: relative; top: 4px;' />: " + _NewCreateDateTime + "<br /><img onmousemove='ShowPopup(event, \"" + dic("TTOS", lang) + " со исклучен мотор\")' onmouseout='HidePopup()' src='../images/sum1.png' style='height: 16px; width: 16px; position: relative; top: 4px;' />: " + _zero + (_diffDT.days != 0 ? (_diffDT.days + " day(s)") : "") + " " + (_diffDT.hours != 0 ? (_diffDT.hours + " h") : "") + " " + (_diffDT.minutes != 0 ? (_diffDT.minutes + " min") : "") + "</strong><br/><br/><strong style='font-size: 12px;'><img onmousemove='ShowPopup(event, \"" + dic("STOS", lang) + "\")' onmouseout='HidePopup()' src='../images/startRec.png' style='height: 17px; width: 16px; position: relative; top: 4px;' />: " + _NewCreateDateTime + "<br /><img onmousemove='ShowPopup(event, \"" + dic("ETOS", lang) + "\")' onmouseout='HidePopup()' src='../images/endRec.png' style='height: 17px; width: 16px; position: relative; top: 4px;' />: " + _NewCreateDateTime2 + "<br /><img onmousemove='ShowPopup(event, \"" + dic("TTOS", lang) + " со вклучен мотор\")' onmouseout='HidePopup()' src='../images/sum1.png' style='height: 16px; width: 16px; position: relative; top: 4px;' />: " + _zero1 + (_diffDT1.days != 0 ? (_diffDT1.days + " day(s)") : "") + " " + (_diffDT1.hours != 0 ? (_diffDT1.hours + " h") : "") + " " + (_diffDT1.minutes != 0 ? (_diffDT1.minutes + " min") : "") + "</strong></div>", null, true);
                this.popup = popup;
        ...
        tmpMarkersRec[tmpMarkersRec.length - 1].events.element.setAttribute("onmousemove", "ShowPopup(event, '<strong style=\"font-size: 12px;\">" + dic("STOS", lang) + ": " + _NewCreateDateTime1 + "<br />" + dic("ETOS", lang) + ": " + _NewCreateDateTime + "<br />" + dic("TTOS", lang) + " со исклучен мотор: " + _zero + (_diffDT.days != 0 ? (_diffDT.days + " day(s)") : "") + " " + (_diffDT.hours != 0 ? (_diffDT.hours + " h") : "") + " " + (_diffDT.minutes != 0 ? (_diffDT.minutes + " min") : "") + "</strong><br/><br/><strong style=\"font-size: 12px;\">" + dic("STOS", lang) + ": " + _NewCreateDateTime + "<br />" + dic("ETOS", lang) + ": " + _NewCreateDateTime2 + "<br />" + dic("TTOS", lang) + " со вклучен мотор: " + _zero1 + (_diffDT1.days != 0 ? (_diffDT1.days + " day(s)") : "") + " " + (_diffDT1.hours != 0 ? (_diffDT1.hours + " h") : "") + " " + (_diffDT1.minutes != 0 ? (_diffDT1.minutes + " min") : "") + "</strong>')");
        $(tmpMarkersRec[tmpMarkersRec.length - 1].events.element).mouseout(function () { HidePopup() });
        tmpMarkersRec[tmpMarkersRec.length - 1].events.element.addEventListener('click', function (e) { AddPOI = true; VehClick = true; });
    }
}
...
function AddLayerRoute(el, num) {
    ...
    $(layerButton).css({ display: 'block', position: 'absolute', zIndex: '7000', backgroundColor: 'transparent', color: '#fff', left: _le + 'px', top: '20px', width: '130px', height: 'auto', textAlign: 'left' });
    layerButton.className = 'text3';
	
	$(layerButton).html('<div id="radioBtnDiv"><input type="radio" runat="server" name="testGroup" value="1" id="test1" checked /><label id="Label1" for="test1" style="cursor:hand" runat="server">' + dic("Routes.Normal", lang) + '</label><input type="radio" runat="server" name="testGroup" value="2" id="test2" /><label id="Label2" for="test2" style="cursor:hand" runat="server">' + dic("Routes.Fast") + '</label></div>');
	$('#test1').button();
    $('#test2').button();
...
function AddLayerSwitcher(el, num) {
	...
	if ((Browser() == 'iPad' && ShowGFBtn) || Routing == true)
		_w = 550;
...
function AddClosePopUpButton(el, num) {
    ...
    $(layerCB).css({ display: 'block', position: 'absolute', zIndex: '7000', backgroundColor: 'transparent', color: '#fff', left: (document.body.clientWidth - 100) + 'px', top: '10px', width: '100px', height: 'auto', textAlign: 'left' });
...
function AddPrintButton(el, num) {
    ...
    $(layerPrint).css({ display: 'block', position: 'absolute', zIndex: '7000', backgroundColor: 'transparent', color: '#fff', left: (document.body.clientWidth - 180) + 'px', top: '10px', width: '100px', height: 'auto', textAlign: 'left' });
...
function drawRecPerDay(_day1, _day2, vh) {
    ...
    $.ajax({
        url: "../report/getHistoryByDay.php?v=" + vh + "&sd=" + _d1 + "&ed=" + _d2,
        context: document.body,
        success: function (data) {
...
function SelectMapLayer(num, l) {
    ...
    if (MapType[num] == 'YAHOOM') { layer = new OpenLayers.Layer.WMS( "OpenLayers WMS", "http://maps.gps.mk:8080/geoserver/gwc/service/wms", {'layers': 'GeonetMaps', format: 'image/png'}); }
...
function CreateMarkerSE_Rec(_lonn, _latt, _ign, _dat, se) {
    ...
    var calculateOffset = function (size) { return new OpenLayers.Pixel(-(size.w / 2), -size.h); };
    if (se == 1 && se == '1') {
        var icon = new OpenLayers.Icon('../images/Start.png', size, null, calculateOffset);
    }
    else {
        var icon = new OpenLayers.Icon('../images/Stop.png', size, null, calculateOffset);
    }
    if (Maps[0] != null) {
        var ll = new OpenLayers.LonLat(parseFloat(_lonn), parseFloat(_latt)).transform(new OpenLayers.Projection("EPSG:4326"), Maps[0].getProjectionObject())
        //if (MapType[0] == 'YAHOOM') { var ll = new OpenLayers.LonLat(parseFloat(_lonn), parseFloat(_latt)) }
...
function CreateMarkerIgnition_Rec(_lonn, _latt, _date, _date1) {
    var size = new OpenLayers.Size(48, 48);
    var calculateOffset = function (size) { return new OpenLayers.Pixel(-(size.w / 2) - 1, -size.h + 3 ); };
    var icon = new OpenLayers.Icon('../images/Parking1.png', size, null, calculateOffset);
    //var _NewCreateDateTime = _date.split(" ")[0].split("-")[0] + "/" + _date.split(" ")[0].split("-")[1] + "/" + _date.split(" ")[0].split("-")[2].substring(2, 4) + " " + _date.split(" ")[1].split(":")[0] + ":" + _date.split(" ")[1].split(":")[1];
    //var _NewCreateDateTime1 = _date1.split(" ")[0].split("-")[0] + "/" + _date1.split(" ")[0].split("-")[1] + "/" + _date1.split(" ")[0].split("-")[2].substring(2, 4) + " " + _date1.split(" ")[1].split(":")[0] + ":" + _date1.split(" ")[1].split(":")[1];
    if (Maps[0] != null) {
        var ll = new OpenLayers.LonLat(parseFloat(_lonn), parseFloat(_latt)).transform(new OpenLayers.Projection("EPSG:4326"), Maps[0].getProjectionObject());
...
function CreateMarkerStartEnd(_lonn1, _latt1, _date1, _lonn2, _latt2, _date2, strDistStartStop) {
	if(parseFloat(strDistStartStop) < 50)
	{
	    ...
	    var icon = new OpenLayers.Icon('../images/startstop.png', size, null, calculateOffset);
	    var imgS = 'startRec';
	    var imgE = 'endRec';
	    ...
	    if (Maps[0] != null) {
	        ...
	        var popup = new OpenLayers.Popup.FramedCloud("Popup",
	            ll,
	            new OpenLayers.Size(500, 500),
	            "<div class='text1' style='overflow: hidden; font-size:.8em; position: relative; left: -2px; top: -3px; width: 125px; height: 55px; margin-right: -15px; margin-bottom: -22px;'><strong style='font-size: 12px;'><img onmousemove='ShowPopup(event, \"" + _textS + "\")' onmouseout='HidePopup()' src='../images/" + imgS + ".png' style='height: 17px; width: 16px; position: relative; top: 4px;' />: " + _date1.split(/\-|\s/)[3].split(":")[0] + ":" + _date1.split(/\-|\s/)[3].split(":")[1] + " " + _date1.split(/\-|\s/)[2] + "-" + _date1.split(/\-|\s/)[1]+ "-" + _date1.split(/\-|\s/)[0] + "</strong><br/><strong style='font-size: 12px;'><img onmousemove='ShowPopup(event, \"" + _textE + "\")' onmouseout='HidePopup()' src='../images/" + imgE + ".png' style='height: 17px; width: 16px; position: relative; top: 4px;' />: " + _date2.split(/\-|\s/)[3].split(":")[0] + ":" + _date2.split(/\-|\s/)[3].split(":")[1] + " " + _date2.split(/\-|\s/)[2] + "-" + _date2.split(/\-|\s/)[1]+ "-" + _date2.split(/\-|\s/)[0] + "</strong></div>", null, true);
	        this.popup = popup;
	        this.popup.contentDiv.style.overflow = 'hidden';
	        map.addPopup(this.popup);
	        this.popup.div.style.opacity = '0.7';
	...
	} else
	{
		var size = new OpenLayers.Size(64, 48);
	    var calculateOffset = function (size) { return new OpenLayers.Pixel(-(size.w / 2) - 2, (-size.h / 2) - 20); };
	    var icon1 = new OpenLayers.Icon('../images/Start.png', size, null, calculateOffset);
	    var icon2 = new OpenLayers.Icon('../images/Stop.png', size, null, calculateOffset);
	    var imgS = 'startRec';
	    var imgE = 'endRec';
	    var _textS = dic("Start", lang) + ":<br />";
	    var _textE = dic("End", lang) + ":<br />";
	    ...
	    if (Maps[0] != null) {
	        ...
	        var popup = new OpenLayers.Popup.FramedCloud("Popup",
	            ll,
	            new OpenLayers.Size(500, 500),
	            "<div class='text1' style='overflow: hidden; font-size:.8em; position: relative; left: -2px; top: -3px; width: 125px; height: 35px; margin-right: -15px; margin-bottom: -22px;'><strong style='font-size: 12px;'><img onmousemove='ShowPopup(event, \"" + _textS + "\")' onmouseout='HidePopup()' src='../images/" + imgS + ".png' style='height: 17px; width: 16px; position: relative; top: 4px;' />: " + _date1.split(/\-|\s/)[3].split(":")[0] + ":" + _date1.split(/\-|\s/)[3].split(":")[1] + " " + _date1.split(/\-|\s/)[2] + "-" + _date1.split(/\-|\s/)[1]+ "-" + _date1.split(/\-|\s/)[0] + "</strong></div>", null, true);
	        this.popup = popup;
	        this.popup.contentDiv.style.overflow = 'hidden';
	        map.addPopup(this.popup);
	        this.popup.div.style.opacity = '0.7';
	        ...
	        var popup = new OpenLayers.Popup.FramedCloud("Popup",
	            ll,
	            new OpenLayers.Size(500, 500),
	            "<div class='text1' style='overflow: hidden; font-size:.8em; position: relative; left: -2px; top: -3px; width: 125px; height: 35px; margin-right: -15px; margin-bottom: -22px;'><strong style='font-size: 12px;'><img onmousemove='ShowPopup(event, \"" + _textE + "\")' onmouseout='HidePopup()' src='../images/" + imgE + ".png' style='height: 17px; width: 16px; position: relative; top: 4px;' />: " + _date2.split(/\-|\s/)[3].split(":")[0] + ":" + _date2.split(/\-|\s/)[3].split(":")[1] + " " + _date2.split(/\-|\s/)[2] + "-" + _date2.split(/\-|\s/)[1]+ "-" + _date2.split(/\-|\s/)[0] + "</strong></div>", null, true);
	        this.popup = popup;
	        this.popup.contentDiv.style.overflow = 'hidden';
	        map.addPopup(this.popup);
	        this.popup.div.style.opacity = '0.7';
                        ...
                        if (SelectedSpliter == 5) {
                            ...
                            if (SelectedSpliter == 6) {
                                ...
                                if (SelectedSpliter == 7) {
                                    ...
                                    _html = _html + '<div id="ActiveW2" onclick="ActiveWindow(this.id);" class="ActiveW" style="background-image:url(../images/activew.png); background-position: -432px 0px;"></div>';
                                    _html = _html + '<div id="ActiveW3" onclick="ActiveWindow(this.id);" class="ActiveW" style="background-image:url(../images/activew.png); background-position: -456px 0px;"></div>';
...
function DrawLine_RouteNew(_lon2, _lat2, _lon1, _lat1, _ordNum, _name, _file, _len) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            {
            	$('#IDMarker_' + PointsOfRoute[_len + 1].id + '_' + _len)[0].children[1].value = Math.round(data.split("^$")[1] * 100)/ 100 + " Km"; //Math.round((parseInt(data.substring(data.indexOf("distance") + 10, data.indexOf("total_time") - 2), 10) / 1000)) + " km";
            	...
            	if($('#txt_alertWait').val() != "/")
            		_min = " min )";
            	$('#IDMarker_' + PointsOfRoute[_len + 1].id + '_' + _len)[0].children[2].value = Sec2Str(data.split("^$")[2]) + " ( " + $('#txt_alertWait').val() + _min; //~ ~(_min / 60) < 1 ? _min + " min" : (~ ~(_min / 60) + " h " + (_min % 60) + " min");
            	var totalkm = 0;
            	var totaltime = 0;
            	for(var bb = 0; bb < $('#MarkersIN')[0].children.length; bb++)
            	{
            		if($('#MarkersIN')[0].children[bb].children[1].value != "/")
            		{
	            		totalkm += parseFloat($('#MarkersIN')[0].children[bb].children[1].value,10);
            			totaltime += Str2Sec($('#MarkersIN')[0].children[bb].children[2].value.substring(0,$('#MarkersIN')[0].children[bb].children[2].value.indexOf("(")-1));
            		}
            		if(bb>0)
            			if($('#MarkersIN')[0].children[bb].children[1].value == "/")
            				chechLoadAll = false;
            	...
            	$('#IDMarker_Total')[0].children[1].value = Math.round(totalkm * 100)/ 100 + " km";
            	$('#IDMarker_Total')[0].children[2].value = Sec2Str(totaltime);
        	}
            var _lon = new Array();
            var _lat = new Array();
            _lon[0] = _lon2;
...
function DrawLine_Route1(_lon2, _lat2, _lon1, _lat1, _ordNum, _sw, _name, _file) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            {
	            $('#IDMarker_' + PointsOfRoute[parseInt(_ordNum, 10) + 1].id + '_' + _ordNum)[0].children[1].value = Math.round(data.split("^$")[1] * 100)/ 100 + " Km"; //Math.round((parseInt(data.substring(data.indexOf("distance") + 10, data.indexOf("total_time") - 2), 10) / 1000)) + " km";
	            //var _min = Math.round((parseInt(data.substring(data.indexOf("total_time") + 12, data.indexOf("start_point") - 2), 10) / 60));
	            var _min = " )";
            	if($('#txt_alertWait').val() != "/")
            		_min = " min )";
	            $('#IDMarker_' + PointsOfRoute[parseInt(_ordNum, 10) + 1].id + '_' + _ordNum)[0].children[2].value = Sec2Str(data.split("^$")[2]) + " ( " + $('#txt_alertWait').val() + _min; //~ ~(_min / 60) < 1 ? _min + " min" : (~ ~(_min / 60) + " h " + (_min % 60) + " min");
	            var totalkm = 0;
	            var totaltime = 0;
	            for(var bb = 0; bb < $('#MarkersIN')[0].children.length; bb++)
	           	{
...
function EnableDisable(_alid, _vehlist) {
    if ($('#menu-4').html() == null) {
        ...
        $.ajax({
            url: "UpdateActive.php?id=" + _alid + "&a=0",
            context: document.body,
            success: function (data) {
                for (var i = 0; i < Vehicles.length; i++) {
...
function EnableDisableDISABLE(_alid, _vehlist) {
    $.ajax({
        url: "UpdateActive.php?id=" + _alid + "&a=1",
        context: document.body,
        success: function (data) {
            //msgbox(data);
