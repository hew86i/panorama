...
function Log(userID) {
    ...
    var vehicle = document.getElementById('txtVeh').name; //($('#txtVehicles').val()).split("#")[0]
    ...
    xmlHttp.open("GET", './addLog.php?_user=' + userID + '&_startDT=' + startDT + '&_endDT=' + endDT + '&_vehicle=' + vehicle + '&_report=' + rep, true);
    xmlHttp.send(null);
    }
  	 ...
  	 vozila.push($(this).val());
        ...
        url: "_admin_PortsToAllVehicles.php?clientid="+clientid,
          ...
          success: function(data){
            ...
            top.$('#div-ports-all-vehicles').dialog({  modal: true, width: 550, height: 370, resizable: false,
                 buttons: {
                    Додади: function(){
                    	...
                    	//return false;
                            $.ajax({
		                        ...
		                        success: function(data){
		                        	if(data!="Ok")
		                         		alert("Успешно додаден темплејт за типови на порти!");
		                         	else
		                         		alert("Не успешно додаден темплејт за типови на порти")
