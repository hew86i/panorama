...
function SettingsMeni(no) { 
    ...
    for (var i=1;i<=8;i++){document.getElementById("menu_set_"+i).className="repoMenu corner5 text2"}
            ...
            $('#div-edit-user').dialog({ modal: true, width: 350, height: 250, resizable: false,
                 buttons: {
			        OK: function() {
                        ...
                        var pass = encodeURIComponent($('#CEPassword').val())
                        ...
                        if(un==''){
                            ...
                            if(fn==''){
	                            ...
	                            url: "UpUser.php?fn="+fn+"&un="+un+"&pass="+pass+"&email="+email+"&id="+id,
	                             context: document.body,
	                              success: function(data){
	                              	alert("Успешно го променивте корисникот !");
 ...
 $('#div-add-user').dialog({ modal: true, width: 350, height: 250, resizable: false,
            buttons: {
				Внеси: function(data) {
                    ...
                    var pass = encodeURIComponent($('#CPassword').val())
                    var fn = $('#CFullName').val()
                    var email = $('#Cmail').val()
                    var golemaBukva = /[A-Z]/.test(pass)
                    var malaBukva = /[a-z]/.test(pass)
                    var specijalenZnak = /[*\W]/.test(pass)
                    	  ...
                    	  if(fn==''){
                                ...
                                if(un==''){
                                    ...
                                    if(golemaBukva==0 && malaBukva==0)
                                            {
                                            	alert("Внесувајте латинични букви.")
                                            }
                ...
                buttons: {
				    Yes: function(lang) {
                      		$.ajax({
		                        ...
		                        context: document.body,
		                        success: function(data){
		                        alert("Успешно го избришавте корисникот !");
		                        window.location.reload();
								}
		                    });	
                            $( this ).dialog( "close" );
...
function AddUserSettings(){
   ...
   // debugger;
    ...
    if( document.getElementById("imperial").checked == true){Kilometri="mph"}
    var show = '1'
...
function LoadData(_i, _j, _equal, _first){
	...
	if (PrevHash == 'menu_set_1') {
	    page = 'USettings.php'
	    document.getElementById('ifrm-cont').src = './USettings.php';
	    
	}

	if (PrevHash == 'menu_set_2') {
	    page = 'CSettings.php'
	     document.getElementById('ifrm-cont').src = './CSettings.php';
	}
	if(PrevHash=='menu_set_3')
	{
		page='GroupPOI.php'
		document.getElementById('ifrm-cont').src = './GroupPOI.php';
	}
	if(PrevHash=='menu_set_4')
	{
		page='Vehicles.php'
		document.getElementById('ifrm-cont').src = './Vehicles.php';
	}
	if(PrevHash=='menu_set_5')
	{
		page='Organisation.php'
		document.getElementById('ifrm-cont').src = './Organisation.php';
	}
	if(PrevHash=='menu_set_6')
	{
		page='Drivers.php'
		document.getElementById('ifrm-cont').src='./Drivers.php';
	}
	if(PrevHash=='menu_set_7')
	{
		page='WorkTime.php'
		document.getElementById('ifrm-cont').src='./WorkTime.php';
	}
	if(PrevHash=='menu_set_8')
	{
		page='Schedulers.php'
		document.getElementById('ifrm-cont').src='./Schedulers.php'
	}
}
