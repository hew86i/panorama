...
var Locale = $H({
  ...
  monthNames: (typeof Date.CultureInfo == 'undefined' ? $w(top.top.dic("Reports.January") + ' ' + top.top.dic("Reports.February") + ' ' + top.dic("Reports.March")+ ' ' + top.dic("Reports.April")+ ' '+ top.dic("Reports.May")+' '+ top.dic("Reports.June") + ' ' +  top.dic("Reports.July") + ' ' + top.dic("Reports.August") + ' ' + top.dic("Reports.September") + ' ' + top.dic("Reports.October") + ' ' + top.dic("Reports.November") + ' ' + top.dic("Reports.December")) : Date.CultureInfo.monthNames),
  dayNames:   (typeof Date.CultureInfo == 'undefined' ? $w(top.dic("Reports.Sunday")+ ' ' + top.dic("Reports.Monday")+ ' ' + top.dic("Reports.Tuesday")+ ' ' + top.dic("Reports.Wednesday")  + ' ' + top.dic("Reports.Thursday") + ' ' + top.dic("Reports.Friday") + ' ' + top.dic("Reports.Saturday")) : Date.CultureInfo.dayNames),
  ...
  weekOffset: (typeof Date.CultureInfo == 'undefined' ? 1 : Date.CultureInfo.firstDayOfWeek)
});
...
var Timeframe = Class.create({
  ...
  parseField: function(fieldName, populate) {
    ...
    if(start.value != "" && fieldName == "start")
    	date = new Date(parseInt(start.value.split("-")[2], 10), parseInt(start.value.split("-")[1], 10)-1, parseInt(start.value.split("-")[0], 10), 12);
	if(end.value != "" && fieldName == "end")
    	date = new Date(parseInt(end.value.split("-")[2], 10), parseInt(end.value.split("-")[1], 10)-1, parseInt(end.value.split("-")[0], 10), 12);
    ...
    if(start.value != "" && end.value != "")
    	if(parent.$('#txtNewDate').length > 0)
    		parent.$('#txtNewDate').val(" " + start.value.split("-")[0] + " " + this.monthNames[parseInt(start.value.split("-")[1], 10)-1].substring(0, 3) + " " + start.value.split("-")[2] + " - " + end.value.split("-")[0] + " " + this.monthNames[parseInt(end.value.split("-")[1], 10)-1].substring(0, 3) + " " + end.value.split("-")[2]);
    return this;
  },
  ...
  handleDateClick: function(element, couldClear) {
    ...
    if (this.stuck) {
      ...
      //alert(element.date) OVA E TOA
      document.getElementById('txtDateRange').selectedIndex = 7;
      if (StartEnd ==0 ) {
      	GoranStart = element.date
      	StartEnd = 1;
      	this.fields.get('end').style.border = '2px solid #FFB03B';
      	this.fields.get('end').classList.add('shadow');
      	this.fields.get('start').classList.remove('shadow');
      	this.fields.get('start').style.border = '';
      } else {    
      	StartEnd = 0;
      	this.fields.get('start').style.border = '2px solid #FFB03B';
      	this.fields.get('start').classList.add('shadow');
      	this.fields.get('end').classList.remove('shadow');
      	this.fields.get('end').style.border = '';
      }
      ...
      if (this.range.get('end') && this.range.get('end').toString() == date)
        this.startdrag = this.range.get('start');
      	...
      	if (StartEnd==1) {
	    	...
	    	if (GoranStart>date) {
			    this.startdrag = this.range.set('start', date);
		    	this.range.set('end', GoranStart);	
	    	} else {
		    	this.startdrag = this.range.set('start', GoranStart);
		    	this.range.set('end', date);
	    	}
	    	
			//document.getElementById('goran').innerHTML = (GoranStart+'<br>'+new Date('2013 March 01 12:00:00 GMT+0100'))
  ...
  eventMouseUp: function(event) {
    ...
    if(start.value != "" && end.value != "")
    	if(parent.$('#txtNewDate').length > 0)
    		parent.$('#txtNewDate').val(" " + start.value.split("-")[0] + " " + tf.monthNames[parseInt(start.value.split("-")[1], 10)-1].substring(0, 3) + " " + start.value.split("-")[2] + " - " + end.value.split("-")[0] + " " + tf.monthNames[parseInt(end.value.split("-")[1], 10)-1].substring(0, 3) + " " + end.value.split("-")[2]);
  },
