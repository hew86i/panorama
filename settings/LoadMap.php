...
<?php
	...
	$dsPp = query("select st_x(st_transform(geom, 4326)) lon, st_y(st_transform(geom, 4326)) lat,type,groupid from pointsofinterest where id =". $idTo);
	...
	{
	$dsPp1 = query("select st_x(st_centroid(geom)) lat, st_y(st_centroid(geom)) lon,groupid from pointsofinterest where id =". $idTo);
...
/*
    ...
    if('<?php echo $type?>' == 1){
    	LoadPOIbyID('<?php echo $idTo?>');
	}else
	{
		DrawZoneOnLiveSettings('<?php echo $idTo?>', '<?php echo $boja2?>');
	}
