 ...
 <body style="margin:0px 0px 0px 0px; padding:0px 0px 0px 0px" onResize="SetHeightLite()">
  ...
  <?php
   /*function Reverse_Array($array) {
	...
	if ($prEmail == true) {
		$emailClick = "AddEmail(6, '$uid', 'Log report', '$cLang', '$cid')";
  ...
  <div id="icons" style="position: relative; right:8px">
	        ...
	        <div onclick="createPDF('LogReport', '<?php echo $uid?>', '<?php echo $cid?>')" <?php echo $pdfStyle?>>
		...
		<div onclick="createXls('LogReport', 'a', '<?php echo $uid?>', '<?php echo $cid?>')" <?php echo $excStyle?>>
...
<br>
  ...
  <div class="corner5" style="width:98%; border:1px solid #bbb; background-color:#fafafa; margin-left:1%">
 	 <div style="padding-left:40px; padding-top:10px; width:500px" class="textTitle">
		...
		$sqln_ = "select fullname, id from users where clientid=" . $cid;
	...
	<table width="94%" border="0" cellspacing="2" cellpadding="2" align="center" style="margin-top:-30px;">
    		<tr>
    				...
    				<div class="side-by-side clearfix" style="float:right">
			           ...
			           <select id = "TipNaKorisnik" class="chzn-select-no-results" tabindex="10" data-placeholder="Изберете корисник" style="font-size: 11px; position: relative; top: 0px; visibility: visible; float: right; font-family:Arial, Helvetica, sans-serif" onchange="OptionsChange()" class="combobox text2">	
	      				...
	      				while ($dr = pg_fetch_array($dsn)) {
	      					...
	      					?>
	      					<option id="<?php echo $dr["id"]?>" <?php echo $selected?> value="<?php echo $dr["id"]?>"><?php echo $dr["fullname"]?></option>
	      				<?php
	      					$cnt ++;
	      				}
    			...
    			<!--td width="22%" style="float: right">
    				<div style="display: inline; float: left;">
    					<span style="float: right; position: relative; top: 7px; visibility: visible;" class="text5"><strong><?php echo dic_("Reports.Period")?>:	</strong></span>
    					
    					</div>
					<div style="display: inline; float: left; margin-left: 10px; margin-top: 2px;">
						<input id="txtNewDate" onclick="opencal(event)" type="text" width="180px" class="textboxCalender corner5 text2" style="width: 200px; height: 24px; cursor: pointer;" value="" />
					</div>
					<img id="imgkal" src="../images/cal.png" onclick="opencal(event)" style="display: inline; float: left; position: relative; margin-top: 2px; left: -25px; cursor: pointer" />
...
<script>
	...
	function OptionsChange(){
		seluser = $('#TipNaKorisnik').find('option:selected').attr("id");
