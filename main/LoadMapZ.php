...
<?php
	...
	$note = dlookup("select note from alarmshistory where datetime='" . $dat . "' and vehicleid=" . $vehID . " and name='" . $type . "'");
...
<table id="live-table" width="100%" border="0" cellpadding="0" cellspacing="0" style="position: relative;">
	<tr>
		<td id="vehicle-list" width="250px" valign="top">
				...
				<div id="scroller">
					...
					<div id="menu-3" class="menu-container" style="width:100%;">
						...
						<div id="menu-container-3" style="width:230px; padding-left:10px; padding-bottom:10px; padding-top:10px; overflow:auto">
							<div>
								<textarea id="alarmZabeleska" disabled="disabled" style="width: 225px; padding: 5px; height: 100px; font-family: arial; font-size: 12px;">
									<?php echo $note?>
								</textarea>
							</div>
						</div>
			...
			<div id="closeAlert" style="position: relative; top: 20px; left: 5px;" onclick="closeAlarmWindow1()"><?php echo dic_("Tracking.Close")?></div>
				<script>
					$('#closeAlert').button();
				</script>
...
<script type="text/javascript">
   ...
   //$('#alarmZabeleska').on('touchstart', function (event) {
    ...
    //LoadPOIbyID('217190');
	if(_type == "enterZone")
		PleaseDrawArea('<?php echo $zoneid?>', '<?php echo $zonecolor?>');
	if(_type == "leaveZone")
		PleaseDrawArea('<?php echo $zoneid?>', '<?php echo $zonecolor?>');
