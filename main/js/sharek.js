...
function dic(key, lang) {
    ...
    aLang[61]="searchByName;Search streets, latitude/longitude, points of interest;Пребарувај улици, latitude/longitude, точки од интерест;Rues de la recherche, latitude/longitude, les points d'intérêt";
...
function SettingsMeni(no) { 
	if ((no==1) && (AllowUSettings == 'false' || AllowUSettings == '0')) {return}
	if ((no==2) && (AllowCSettings=='false' || AllowCSettings == '0')) {return}
    ...
    if (no == 1) {
        ShowWait();
        $('#stt-cont').css({display: 'block'});
        $('#ifr-map').css({display: 'none'});
        $.ajax({
            url: "./Admin.php?l="+lang,
            context: document.body,
            success: function (data) {
                HideWait();
                $('#stt-cont').html(data)
        ...
        if(no == 2){
            ...
            $.ajax({
                ...
                if(no == 3){
                    ...
                    $('#ifr-map').css({display: 'none'});
                    $.ajax({
                        url: "VehicleSettings.php?l="+lang,
                        context: document.body,
                        success: function (data) {
                            HideWait()
                         	$('#stt-cont').html(data)
                    ...
                    if(no == 4){
                        ...
                        $('#stt-cont').css({display: 'none'});
                        $('#ifr-map').css({display: 'block'});
                        document.getElementById('ifr-map').src="./LoadMap.php?l=" + lang;
                    }
                } 
            }
        }
...
function AddUserSettings(){
    ...
    var DefMap = '0'
    if(document.getElementById("DMRadio1").checked == true){DefMap=1}
    if(document.getElementById("DMRadio2").checked == true){DefMap=2}
    if(document.getElementById("DMRadio3").checked == true){DefMap=3}
    if(document.getElementById("DMRadio4").checked == true){DefMap=4}
    ...
    var TimeTrack =0
    TimeTrack = $('#TimeTrack').val()
    var Kilometri ='0'
    if( document.getElementById("metric").checked == true){Kilometri=1}
    if( document.getElementById("imperial").checked == true){Kilometri=2}
    var show = '1'
    ...
    cena_km = $('#cena_km').val()
    var string = "SaveUserSettings.php?AppSettings="+AppSettings+"&DefLanguage="+DefLanguage+"&DateFormat="+DateFormat+"&AMapsGoogle="+AMapsGoogle+"&AMapsOMS="+AMapsOMS+"&AMapsBing="+AMapsBing+"&AMapsYahoo="+AMapsYahoo+"&DefMap="+DefMap+"&TimeFormat="+TimeFormat+"&EngineON="+EngineON+"&EngineOFF="+EngineOFF+"&EngineOFFPassengerON="+EngineOFFPassengerON+"&SatelliteOFF="+SatelliteOFF+"&TaximeterON="+TaximeterON+"&TaximeterOFFPassengerON="+TaximeterOFFPassengerON+"&PassiveON="+PassiveON+"&Satellite="+Satellite+"&datetime="+datetime+"&speed="+speed+"&location="+location+"&poi="+poi+"&zone="+zone+"&passengers="+passengers+"&taximeter="+taximeter+"&fuel="+fuel+"&TimeTrack="+TimeTrack+"&Kilometri="+Kilometri+"&show="+show+"&start="+start+"&km_start="+km_start+"&cena_km="+cena_km;
	
    $.ajax({
        url: string, //"SaveUserSettings.aspx?AppSettings="+AppSettings+"&DefLanguage="+DefLanguage+"&DateFormat="+DateFormat+"&AMapsGoogle="+AMapsGoogle+"&AMapsOMS="+AMapsOMS+"&AMapsBing="+AMapsBing+"&AMapsYahoo="+AMapsYahoo+"&DefMap="+DefMap+"&TimeFormat="+TimeFormat+"&EngineON="+EngineON+"&EngineOFF="+EngineOFF+"&EngineOFFPassengerON="+EngineOFFPassengerON+"&SatelliteOFF="+SatelliteOFF+"&TaximeterON="+TaximeterON+"&TaximeterOFFPassengerON="+TaximeterOFFPassengerON+"&PassiveON="+PassiveON,
        context: document.body,
        success: function(data){
