...
<?php
	...
	$cid=2;
	$uid = 262;
	//$uid=dlookup("select clientid from vehicles where id=2");
	
    $DefMap = nnull(dlookup("select defaultmap from users where id=".$uid),1);
    if($DefMap == "GOOGLEM")
	{
		?>
	...
	$tzone = $tzone - 1;
	$DFormat1 = "dd-MM-yyyy"; //nnull(dlookup("select DateFormat from usersettings where userid=" . $uid),"dd-MM-yyyy");
	$TFormat1 = "24 Hour Time"; //nnull(dlookup("select TimeFormat from usersettings where userid=" . $uid),"24 Hour Time");
	$TFormat = "";
	$DFormat = "";
	...
	$address = dlookup("select latloninarea(" . $sLat . ", " . $sLon . ", " . $cid . ")");
...
<table id="live-table" width="100%" border="0" cellpadding="0" cellspacing="0" style="position: relative;">
	<tr>
		<td id="vehicle-list" width="250px" valign="top">
			...
			<div id="cancelAlert" style="position: relative; top: 20px; left: 16px;" onclick="LiveTracking()">Следење во живо</div>
				<script>
					$('#closeAlert').button();
					$('#cancelAlert').button();
				</script>
...
<script type="text/javascript">
   ...
   //$('#alarmZabeleska').on('touchstart', function (event) {
	...
	function LiveTracking()
	{
		...
		{
			top.document.location.href = '../tracking/';
			setTimeout("FindVehicleOnMap0(6)", 10000);
		}
	}
</script>